...
public class TypeToJarMapper {
    ...
    public static Map<String, String> createTypeUriToCoordMap(final List<Pair<MavenCoordinate, File>> jars) {
        ...
        for (final var dep : jars) {
            ...
            putAllTypes(result, coord, depJarFile);
        }
        return result;
    }

    private static void putAllTypes(final Map<String, String> result, String coord,
                                    final JarFile depJarFile) {
    ...
    public static JarFile jarOrThrow(final File depJar) {
        try {
