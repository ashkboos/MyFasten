...
public class JSONUtils {
    ...
    private static void appendGraph(StringBuilder result, final DirectedGraph graph) {
        ...
        for (final var edge : graph.edgeSet()) {
            result.append("[").append(edge.firstLong()).append(",").append(edge.secondLong())
                .append("],");
        }
    ...
    private static void appendCall(StringBuilder result,
                                   final Map.Entry<LongLongPair, Map<Object, Object>> entry) {
        result.append("[").append(quote(String.valueOf(entry.getKey().firstLong()))).append(",");
        result.append(quote(String.valueOf(entry.getKey().secondLong()))).append(",{");
