...
import eu.fasten.core.data.callableindex.SourceCallSites.SourceMethodInf;
import eu.fasten.core.data.callableindex.SourceCallSites.CallSite;
import eu.fasten.core.data.callableindex.SourceCallSites.CallSite.InvocationInstruction;
...
public class RocksDao implements Closeable {
    ...
    private InvocationInstruction transformCallType(eu.fasten.core.data.metadatadb.codegen.enums.CallType type) {
        switch (type) {
            case dynamic:
                return InvocationInstruction.DYNAMIC;
            case special:
                return InvocationInstruction.SPECIAL;
            case static_:
                return InvocationInstruction.STATIC;
            case interface_:
                return InvocationInstruction.INTERFACE;
            case virtual:
                return InvocationInstruction.VIRTUAL;
            default:
                return null;
        }
    }
    ...
    public void saveToRocksDb(final ExtendedGidGraph extendedGidGraph) throws IOException, RocksDBException {
        ...
        final Long2ObjectOpenHashMap<List<SourceCallSites.CallSite>> map = new Long2ObjectOpenHashMap<>();

        final Map<Long, String> gidToUriMap = extendedGidGraph.getGidToUriMap();

        // Gather data by source and store it in lists of SourceCallSites.CallSite.
        edgesInfo.forEach((pair, record) -> map.compute(pair.getFirst().longValue(), (k, list) -> {
            if (list == null) list = new ArrayList<>();
            list.add(new SourceCallSites.CallSite(record.getLine(), transformCallType(record.getCallType())
        ...
        for (final LongIterator iterator = graph.iterator(); iterator.hasNext();) {
            ...
            final List<SourceCallSites.CallSite> list = map.get(node);
            ...
            if (list == null) writeLong(0, fbaos); // no data
                ...
                for (final var r : list) {
                    ...
                    writeLong(r.invocationInstruction.ordinal(), fbaos);
                    writeString(r.targetSignature, fbaos);
     ...
     * @throws RocksDBException if there was problem retrieving data from RocksDB
     */
    public SourceCallSites getGraphMetadata(final long index, final DirectedGraph graph) {
        byte[] metadata = getMetaData(index);
        if (metadata == null) {
            return null;
        }
        final Long2ObjectOpenHashMap<SourceMethodInf> map = new Long2ObjectOpenHashMap<>();
        ...
        return new SourceCallSites(map);
    }
