...
public class CallableIndexChecker implements Runnable {
    ...
    private String extractMetaDataForArtifact(SourceCallSites graphMetadata) {
        String delim = "";
        StringBuilder strMetadata = new StringBuilder();
        for (final var key :
            graphMetadata.sourceId2SourceInf.keySet()) {
            strMetadata.append(delim);
            delim = ",\n\n";

            final var value = graphMetadata.sourceId2SourceInf.get(key.longValue());
            strMetadata.append(key).append(" -> ")
                .append(value.sourceSignature()).append(",\n\t")
                .append(value.sourceType()).append(",\n\t")
                .append(value.callSites);
        }
        strMetadata.append("\n");
